...
import {
    getPriority,
    parseDynamicFileF2,
    parseDynamicHighValue,
    parseDynamicSheet,
    parseDynamicSheetCharge,
    parseDynamicSheetDHL
...
export class ShipmentsService {
    ...
    async addConsMasterBySubsidiary(
        ...
        const shipmentsToGenerateIncomes: {
            shipment: Shipment;
            timestamp: Date;
    ...
    private async logUnusualCodes(unusualCodes: {
        trackingNumber: string;
        derivedCode: string;
        exceptionCode?: string;
        eventDate: string;
                        ...
                        for (const shipment of shipmentList) {
                            ...
                            if (exceptionCode && !rules.allowedExceptionCodes.includes(exceptionCode) && mappedStatus !== ShipmentStatusType.ENTREGADO) {
                                ...
                                if (exceptionCode === '03' && rules.allowException03) {
                                    ...
                                    shipmentsWithError.push({
                                        trackingNumber,
                                        reason: `exceptionCode=${exceptionCode} no permitido`,
                                        shipmentId: shipment.id
                                    });
                                    continue;
                                }
                            ...
                            if (!rules.allowedStatuses.includes(mappedStatus) && mappedStatus !== ShipmentStatusType.ENTREGADO) {
                                ...
                                shipmentsWithError.push({
                                    trackingNumber,
                                    reason: `Estatus ${mappedStatus} no permitido`,
                                    shipmentId: shipment.id
                                });
                                continue;
                            }
                            ...
                            if (shouldPersist) {
                                ...
                                // Validar ingresos
                                let incomeValidationResult: IncomeValidationResult = {
                                    isValid: true,
                                    timestamp: eventDate
                                };
                                ...
                                if (([ShipmentStatusType.ENTREGADO, ShipmentStatusType.NO_ENTREGADO].includes(toStatus) || (exceptionCode === '07' && rules.allowIncomeFor07)) &&
                                    ...
                                    incomeValidationResult = {
                                        isValid: false,
                                        timestamp: eventDate,
                                        reason: 'Exception code 03 blocks income generation'
                                    };
                                }
                                ...
                                try {
                                    await this.shipmentRepository.manager.transaction(async (em) => {
                                        ...
                                        if ((toStatus === ShipmentStatusType.ENTREGADO || (toStatus === ShipmentStatusType.NO_ENTREGADO && exceptionCode === '07')) &&
                                            ...
                                            !incomeValidationResult.isValid &&
                                            shipment.id === representativeShipment.id) {
                                            shipmentsWithInvalidIncome.push({
                                                trackingNumber,
                                                eventDate: eventDate.toISOString(),
                                                shipmentId: shipment.id
                                            });
